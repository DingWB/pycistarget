    ...
    **kwargs:
    ...
    if not os.path.exists(path_to_regions_fasta):
        ...
        for key in region_sets.keys():
            ...
            names=region_sets[key].df.apply(lambda x:'-'.join(x.astype(str).tolist()),axis=1).tolist()
            pr_sequence_list.append(pd.DataFrame([names, fa_list], index=['Name', 'Sequence'], columns=names))
...
@ray.remote
def run_cluster_buster_for_motif_ray(cluster_buster_path: str,
                                fasta_filename: str,
                                motif_filename: str,
                                motif_name: str,
                                i: int,
    ...
    return run_cluster_buster_for_motif(cluster_buster_path,fasta_filename,motif_filename,motif_name,i,nr_motifs,verbose)
